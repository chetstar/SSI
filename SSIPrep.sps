...
begin program.
import os
for root, dirs, files in os.walk('k:/meinzer/projects/ssi/pending'):
   for afile in files:
      s=root+'/'+afile
      if 'master' in s:
         os.remove(s)
...
files2=os.listdir('k:/meinzer/projects/ssi/pending')
for file in files2:
   if 'IAR P' in file:
...
for root, dirs, files in os.walk('k:/meinzer/projects/ssi/pending'):
  for afile in files:
   ...
   filename= os.path.splitext("K:/Meinzer/Projects/SSI/%s" % afile)[0]
   tempfile='k:/temp/correctedx.csv'
   spssfile='K:/Meinzer/Projects/SSI/SPSS/%s.sav' % stripfile
   CorrectedFile='K:/Meinzer/Projects/SSI/%s.csv' % stripfile 
...
string datetest(a35).
compute datetest='%s'.
compute SSIindex=index(datetest,'SSI')+4.
compute SSIcountyReportAA=date.dmy(number(substr(datetest,ssiindex+3,2),f2.0),number(substr(datetest,ssiindex,2),f2.0),number(concat('20',substr(datetest,ssiindex+6,2)),f4.0)).
format SSIcountyReportAA (date11).
save outfile='%s.'  """ % (filename,FileName, spssfile)

   spss.Submit(cmdspss)
s='add files\n'
for root, dirs, files in os.walk('k:/meinzer/projects/ssi/spss'):
   if len(files) >1:
      ...
      cmdadd="""
      %s.
      save outfile='k:/meinzer/projects/ssi/combinedexcelfiles.sav'
      """ % s
   elif len(files) ==1:
      cmdadd=("save outfile='k:/meinzer/projects/ssi/combinedexcelfiles.sav'")
   elif len(files) ==0:
      print 'no files'
      cmdadd=('freq ssn')
      break
...
import shutil
spsspath='k:/meinzer/projects/ssi/SPSS'
...
for root, dirs, files in os.walk('k:/meinzer/projects/ssi/pending'):
  for afile in files:
   shutil.move('k:/meinzer/projects/ssi/pending/'+afile,'k:/meinzer/projects/ssi/pending processed/'+afile)

end program.


get file='k:/meinzer/projects/ssi/combinedexcelfiles.sav'.
...
SAVE TRANSLATE /TYPE=ODBC
  ...
  /MISSING=IGNORE
  /TABLE='SPSS_TEMP'
  /KEEP=SSN, Name, nameFirst, nameLast, datetest, SSIindex, SSIcountyReportAA
  /SQL='INSERT INTO `IAR Cases Pending` (SSN, Name, nameFirst, nameLast, datetest, SSIindex, '+
    'SSIcountyReportAA) SELECT SSN, Name, nameFirst, nameLast, datetest, SSIindex, '+
...
save outfile='I:\SSI\SSIqueue\JustMaxedSSI.sav' /keep ssnSSI SSIcountyReportAA Name NameLast NameFirst.

*need to have generic, non-date name since file will be used in other codes*.
 * save outfile='I:\SSI\SSIqueue\IARpendingSSI.sav' /keep ssnSSI SSIcountyReportAA Name NameLast NameFirst.
...
match files /table='I:\SSI\SSIqueue\JustMaxedSSI.sav' /file=* /by ssnSSI
 /keep
ssnSSI
NameLast
NameFirst
